...
(define (key n)
  (let ([e (cadr (get-nth-quadruple n))] [f (caddr (get-nth-quadruple n))] [mylist (list a1 b1 c1 a2 b2 c2 a3 b3 c3)])
   (foldr (λ (x result) (cons (modulo (x e f) 27) result)) '() mylist))
  )
...
(define (message->codes message)
  (map (λ(x) (helper x)) (map char->integer (string->list message)))
  )
...
(define (codes->message codes)
  (list->string (map integer->char (map (λ(x) (helper-2 x)) codes)))
  )
...
(define (extend-key key size)
  (let value ([curr key])
    (cond
      ((<= (length curr) size) (value (append curr key)))
      ((= (length curr) size) curr)
      ((>= (length curr) size) (take curr size))))
  )
...
(define gen-function
  (λ(func x y)
    (let ([extended-key (extend-key y (length x))])
      (map func x extended-key))))

(define encrypt-codes
  (λ(string key)
    (gen-function encrypt string key))
  )
(define decrypt-codes
  (λ(string key)
    (gen-function decrypt string key))
  )
...
(define encrypt-message
  (λ(string key)
    (let* ([string-to-integer (message->codes string)] [encrypted (encrypt-codes string-to-integer key)])
      (codes->message encrypted)))
  )
(define decrypt-message
  (λ(string key)
    (let* ([integer-to-string (message->codes string)] [decrypted (decrypt-codes integer-to-string key)])
      (codes->message decrypted)))
  )