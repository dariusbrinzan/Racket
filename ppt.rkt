...
(define (get-level n p)
  (if (<= n p)
      0
      (+ (get-level (- n p) (* 3 p)) 1))
  )

(define (get-transformations-util n k l)
  (if (= k 0)
      l
      (if (= (modulo n 3) 0)
          (get-transformations-util (quotient n 3) (- k 1) (cons 3 l))
          (get-transformations-util (+ (quotient n 3) 1) (- k 1) (cons (modulo n 3) l))
      )
  )
)
...
(define (get-transformations n)
  (get-transformations-util (update-n n 1) (get-level n 1) '()))
    ...
    (λ(function)
      (λ(tuple)
        (define new-function
          (cond
            ((null? l) tuple)
            ((= 1 (car l)) (car function))
            ((= 2 (car l)) (cadr function))
            ((= 3 (car l)) (caddr function))))
        (if (null? l)
            tuple
            (((get-nth-tuple (cdr l)) function) (apply-functional-transformations (list new-function) tuple))))
      )
    )
  )
...
(define get-nth-ppt-from-matrix-transformations
  (λ(x)
    (((get-nth-tuple (get-transformations x)) (list FT1 FT2 FT3)) '(3 4 5)))
  )
...
(define get-nth-quadruple
  (λ(x)
    (((get-nth-tuple (get-transformations x)) (list FQ1 FQ2 FQ3)) '(1 1 2 3)))
  )
...
(define (c x)
  (+ (* (cadr x) (cadr x)) (* (caddr x) (caddr x))))

(define resolve
  (λ(init l)
      (cons (a init) (cons (b init) (cons (c init) l)))))

(define get-nth-ppt-from-GH-quadruples
  (λ(x)
    (resolve (get-nth-quadruple x) '()))
  )
